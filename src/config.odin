...
load_spall_file :: proc(loader: ^Loader, trace: ^Trace, file_name: string) {
	...
	if magic == spall_fmt.MANUAL_MAGIC {
		...
		if hdr.version < 3 {
			post_error(trace, "Support for auto-tracing v%d has been dropped in this version, please grab the new header!",
				              hdr.version)
			return
		}
		if hdr.version != 3 {
			post_error(trace, "Spall version %d for %s is invalid!",
                              hdr.version, file_name)
			return
		}
